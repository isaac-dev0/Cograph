...
/**
 * Service responsible for managing repository data operations.
 * Handles CRUD operations, syncing, and querying of GitHub repositories stored in the database.
 *
...
export class RepositoriesService {
  ...
  async addRepositoriesToProject(
    ...
    const newIds = repositoryIds.filter(
      (id) => !existing.some((pr) => pr.repositoryId === id),
    );
